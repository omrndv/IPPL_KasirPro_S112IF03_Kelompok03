...
<head>
    ...
    <title>Verifikasi OTP - KasirPro</title>
            ...
            <div class="hidden lg:block">
                <div class="max-w-2xl">
                    ...
                    <p class="mt-6 max-w-xl text-lg leading-8 text-slate-600">
                        Masukkan kode OTP 6 digit yang telah kami kirimkan ke email <span class="font-bold text-brand-600">{{ $email }}</span>. Periksa juga folder Spam jika email tidak muncul di kotak masuk utama.
                    </p>
                </div>
            </div>
            ...
            <div class="w-full lg:justify-self-end">
                <div class="mx-auto w-full max-w-md lg:mx-0">
                    <div class="mb-7 text-center lg:text-left">
                        ...
                        <p class="mb-3 text-xs font-black uppercase tracking-[0.22em] text-brand-600">OTP Verification</p>
                        <h1 class="text-3xl font-black tracking-tight text-ink sm:text-4xl">Verifikasi Kode</h1>
                        <p class="mt-3 leading-7 text-slate-500">Silakan periksa email Anda dan masukkan kode verifikasi.</p>
                    </div>
                    ...
                    <div class="rounded-[2rem] border border-white/80 bg-white/90 p-6 shadow-soft backdrop-blur sm:p-8">
                        @if (session('success'))
                            <div class="mb-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800" role="alert">
                                <span class="font-black">Terkirim!</span> {{ session('success') }}
                            </div>
                        @endif

                        @if (session('error'))
                            <div class="mb-6 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-800" role="alert">
                                <span class="font-black">Salah!</span> {{ session('error') }}
                            </div>
                        @endif
